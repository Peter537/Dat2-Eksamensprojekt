...
public class Plate3D {
    ...
    private Geometry3D createPlate() throws DatabaseException {
        ...
        Geometry3D hole = csg.cylinder3D((double) plateType.getWidth() *0.5, (double) plateType.getThickness() / 2, 50, false);

        // Subtract the holes from the box to create plate with holes
        for(int i = 0; i < this.numberOfHoles; i++) {
            double holePosition = (double) (partsList.getLengthOfPlate() * 10 - plateType.getWidth()) / 2 - i * (partsList.getLengthOfPlate() * 10 - plateType.getWidth()) / (this.numberOfHoles - 1);
            Geometry3D holeAtPosition = csg.translate3D(holePosition, 0, (double) plateType.getThickness()/2).transform(hole);
